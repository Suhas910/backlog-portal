...
// With no user interaction for this long, don't refresh: let the session lapse into a clean
// re-login — a real idle timeout.
const IDLE_LIMIT_MS = 30 * 60 * 1000;

/**
 * Keeps an *active* session from hard-expiring at the token's 1h mark: while a protected page is
 * mounted, schedules a refresh just before expiry, but only if the user interacted recently —
 * otherwise the session lapses and redirects to `<login>?expired=1` for a graceful re-login.
 *
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      ...
      timerRef.current = setTimeout(async () => {
        if (cancelled) return;
        // idle too long — lapse the session rather than silently extend it
